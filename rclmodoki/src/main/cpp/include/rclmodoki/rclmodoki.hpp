...
class MessageType {
public:
  MessageType(eprosima::fastdds::dds::TopicDataType *message_type, std::function<void *(MessageKt &&)> &&kt_to_cpp,
              std::function<jobject(MessageCpp &&)> &&cpp_to_kt)
  ...
  std::function<void *(MessageKt &&)> kt_to_cpp;
  std::function<jobject(MessageCpp &&)> cpp_to_kt;
  JNIClass *jni_class;
};
